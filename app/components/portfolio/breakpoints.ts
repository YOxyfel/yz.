...
export const HOME_SCROLL_BOOTSTRAP_SCRIPT = `(function(){try{if(document.documentElement.dataset.mobileStatic==='on')return;var path=location.pathname;var isHome=path==='/'||/^\\/[a-z]{2}$/.test(path);if(!isHome)return;if('scrollRestoration' in history)history.scrollRestoration='manual';var hash=location.hash;if(hash&&hash!=='#explore')return;if(hash==='#explore'){history.replaceState(null,'',path+location.search);}window.scrollTo(0,0);}catch(e){}})();`
