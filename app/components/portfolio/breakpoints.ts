...
export const HOME_SCROLL_BOOTSTRAP_SCRIPT = `(function(){try{var path=location.pathname;var isHome=path==='/'||/^\\/[a-z]{2}$/.test(path);if(!isHome)return;if('scrollRestoration' in history)history.scrollRestoration='manual';if(location.hash){history.replaceState(null,'',path+location.search);}window.scrollTo(0,0);document.documentElement.scrollTop=0;document.body&&(document.body.scrollTop=0);}catch(e){}})();`
